...
const EditContentForm = ({ contentToEdit, contributors = [] }) => {
  ...
  // Populate form state when `contentToEdit` is available
  useEffect(() => {
    if (contentToEdit) {
      setFormData({
        title: contentToEdit.title || '',
        slug: contentToEdit.slug || '',
        ai_summary: contentToEdit.ai_summary || '',
        body: contentToEdit.body || '',
        content_type: contentToEdit.content_type || '',
        media_url: contentToEdit.media_url || '',
        pdf_url: contentToEdit.pdf_url || '',
        media_duration_sec: contentToEdit.media_duration_sec || '',
        banner_image_url: contentToEdit.banner_image_url || '',
        meta_description: contentToEdit.meta_description || '',
        meta_keywords: contentToEdit.meta_keywords || '',
        publish_date: contentToEdit.publish_date ? new Date(contentToEdit.publish_date).toISOString().slice(0, 16) : '',
        status: contentToEdit.status || 'draft',
        featured: contentToEdit.featured || false,
        popular: contentToEdit.popular || false,
        hero: contentToEdit.hero || false,
        theme_ids: contentToEdit.themes?.map(t => t._id) || [],
        sub_theme_ids: contentToEdit.sub_themes?.map(st => st._id) || [],
        industry_ids: contentToEdit.industries?.map(i => i._id) || [],
        exec_role_ids: contentToEdit.exec_roles?.map(er => er._id) || [],
        contributor_ids: contentToEdit.contributors?.map(c => c.contributor_id._id) || [],
      });
    }
  }, [contentToEdit]);

  // Clear content update error on form field change
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const resultAction = await dispatch(
        updateContent({ id: contentToEdit._id, data: payload })
      );
  ...
  return (
    ...
    >
      ...
      <button type="submit" className="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700" disabled={contentLoading}>
        {contentLoading ? 'Updating...' : 'Update Content'}
      </button>
    </form>
  );
};
